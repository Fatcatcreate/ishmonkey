...
import sys
import zlib
argparser = argparse.ArgumentParser(description="ishan is a monkey")
...
def repo_path(repo, *path):
    """retruns the path under the git directyoiyr"""
...
#chec if path doesnt exiust or empty 
...
def repo_find(path=".", required=True):
    ...
    if parent == path:
        # Bottom case
        # os.path.join("/", "..") == "/":
        # If parent==path, then path is root and we did't fine a .git.
        if required:
...
class GitObject (object):
    ...
    def serialize(self, repo):
        """implemented in subelcasses as needs to rtake byte string from data and convert
        """
...
def tree_parse_one(raw, start=0):
    ...
    # Find the null termintor of the path
    y = raw.find(b'\x00', x)
    # and read the path
    path = raw[x+1:y]
...
def tree_parse(raw):
    ...
    max = len(raw)
    ret = list()
    #take tree and parse each posotion untill the end
    while pos < max:
        pos, data = tree_parse_one(raw, pos)
        ret.append(data)

    return ret
#basically the tree after weve parsed it in according to the structure 
# two trees one sorted correctykly items and the other not will have 
# diffrent hashes breaking git rule as same contents
def tree_leaf_sort_key(leaf):
    if leaf.mode.startswith(b"10"):
        return leaf.path
    else:
        return leaf.path + "/"
# sort items by trasnforming them using th sort and then writing 
# it back in using the format for trees
def tree_serialize(obj):
    ...
    return ret
#class for trees how to desrliaaze back into ojbvets, 
# serialose into SHA and init 
...
#shows working directories ion tree 
...
def cmd_checkout(args):
    repo = repo_find()
    # find repo then read the object type and what it is then find tree
    # and check out
...
#for the item in the worktree directory find the object and
#  its destination  if its a ttree make a directory at 
# desiynation adn then ceckout that branch otherwise
#  if its a stanrd object write in bianry and then commit
#  eg go through eacn directory adn write conetnst to local commit 
...
#refrence basically instaed of full SHA identifier 
# just have a text file ref of thre hash or can refer to another refernce
...
def ref_resolve(repo, ref):
    ...
    #BaiscLLT GO DOWBN the chain of references to the original SHA hash
    if not os.path.isfile(path):
        return None
...
# Tagging is like aliasing: a tag introduces a new way to refer
#  to an existing object. After the tag is created,
#  the name v12.78.52 refers to 6071c08
...
#either list or create depending on whether name
def cmd_tag(args):
    repo = repo_find()
...
def tag_create(repo, name, ref, create_tag_object=False):
    ...
    if create_tag_object:
        ...
        tag.kvlm[b'tagger'] = b'ishmonkey <ishmonkey@example.com>'
        # …and a tag message!
        tag.kvlm[None] = b"A tag generated by ishmonkey, which won't let you customize the message!\n"
...
def ref_create(repo, ref_name, sha):
    with open(repo_file(repo, "refs/" + ref_name), 'w') as fp:
        fp.write(sha + "\n")
#Barnch
#basically just a refrence to commit
#eg this brnahc refers to the commit two days ago when i broke my code
#its good its a branch cuz it can be checked out
#HEAD is last commit in branch
#whenever you commit, Git actually does this:
#a new commit object is created, 
# with the current branch’s (commit!) ID as its parent;
#the commit object is hashed and stored;
#the branch ref is updated to refer to the new commit’s hash.

#the current branch It’s a ref file outside of the refs hierarchy,
#  in .git/HEAD, which is an indirect ref 
#detached HEAD state means niot on any brnahc after checkouit 
...
argsp.add_argument("--ishmonkey-type",
                   metavar="type",
                   dest="type",
                   choices=["blob", "commit", "tag", "tree"],
                   default=None,
...
#Basically solve refrences ro find the tiem from the ref i think?
...
#when you do the git add and git rm you are basiucally 
#in the staging of a commit 
#this is stored in an index file 
#the index is just a list of files
#therefore git status doesnt need to somopare files with the index 
# just the last edited to work out what is chnaged 
#When you commit, what git actually does is turn the index file 
# into a new tree object.
#index file is 
# An header with the format version number
#  and the number of entries the index holds;
...
#after freading the 12 byte header 
#we just parse in ecah entry 
#basically we read ijn the metadata thats 
# at specific place sna dt turn it to an int from a bit 
...
def index_read(repo):
    ...
    assert version == 2, "ishmonkey only supports index file version 2"
...
#git ls files displays all the files names in the staging area 
...
def cmd_ls_files(args):
    ...
    for e in index.entries:
        print(e.name)
        #verbose prints out all of the details all the bits 
        #basically if ur a nerd 
        if args.verbose:
...
#GITIGBNORE THE BANE OF MY LIFE basicallty it takes in rules and 
# reads from tje repo, for all the paths it checks if its in the
#  paths to ignore 
...
#Lines that begin with an exclamation mark ! negate the pattern 
...
def gitignore_parse(lines):
    ret = list()
# for all of ther lines if its truw to ignore 
# it appends to ignore oetherwise it doesn
    for line in lines:
...
class GitIgnore(object):
    absolute = None
    scoped = None
#scoped is th efiles in the index eg the gitignore files
#absolute is the files in the repo eg the ones in ~/.config/git/ignore
...
#chec_ignore is mathses a path against the
#  rules if ir matches return  the rukles 
# from git ignore else use absolute rukes 
def check_ignore1(rules, path):
    ...
    for (pattern, value) in rules:
        if fnmatch(path, pattern):
            result = value
    return result
#t just starts at the path’s directory 
# then moves up to the parent directory, recursively,
#  until it has tested root. 
# Notice that this function (and the next two as well), 
# never breaks inside a given .gitignore file. 
# Even if a rule matches, they keep going through the file, 
# because another rule there may negate reverse the effect
...
def check_ignore_absolute(rules, path):
    ...
    for ruleset in rules:
        ...
        if result != None:
            return result
    return False 
#binds the function together
def check_ignore(rules, path):
    if os.path.isabs(path):
        raise Exception("relative to root")
    #if no scoped rules under the .gitignore path then 
    # check against the absolute rules
...
#git status just terlls you what has been added changed/ modified
#compaeres with HEAD eg the SHA of the last commit
#takes no argument 
...
#first need to find whther were on a bracnh or just floating about 
#we check .git/HEAD
def branch_get_active(repo):
    with open(repo_file(repo, "HEAD"), "r") as f:
        head = f.read()
        #if its he correct thing we return rleative path 
...
def cmd_status_branch(repo):
    ...
    if branch:
        #if its not true eg on a branch yay
        print(f"On branch {branch}.")
    else:
        # otherwise detached head
        print(f"HEAD detached at {object_find(repo, 'HEAD')}")
#flatten tree 0 clue what that means and hwther
#  it relevant but sounds cool

#ignore me, we raed the HEAD tree, eg the series of refs
#and then convert to hashmap
...
#compares the index with the HEAD 
# and comparee with each entry in the index
# then by finding the difference are the ones we return
#basiucally as it didnt exist before its added and
#  if SHA not same then modified
...
def cmd_status_index_worktree(repo, index):
    ...
    # We now traverse the index, and compare real
    #  files with the cached
    # versions.
    for entry in index.entries:
...
#now we needs the good ones add and rm
#index read in reverse
...
#rm is remove 
#scary
#checks if in index and in worktree
#then dleetd it
...
#adding 
#hash file into a blob, create index entry
# writre modified index to index file
...
#covert index into tree
# stire coimmit 
# first the gitconfig toi find whi created it
def gitconfig_read():
...
# index is flat we need a tree
#Build a dictionary (hashmap) of directories.
#  Keys are full paths 
#  values are list of GitIndexEntry — files in the directory.
#Traverse this list, going bottom-up, that is, 
# from the deepest directories up to root
#sorted by lentgh of path
# xample, imagine we start at assets/sprites/monsters/
#At each directory, we build a tree with its contents, 
# say cacodemon.png, imp.png and baron-of-hell.png.
#We write the new tree to the repository.
#We then add this tree to this directory’s parent. 
# Meaning that at this point, assets/sprites/ now contains
#  our new tree object’s SHA-1 id under the name monsters.
#And we iterate over the next directory, 
# let’s say assets/sprites/keys where we find red.png,
# blue.png and yellow.png, create a tree, store the tree,
# add the tree’s SHA-1 under the name keys under assets/sprites/, 
# and so on.
